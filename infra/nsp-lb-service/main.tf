...
locals {
  ...
  tags = {
    ...
    GithubRepo = "github.com/${var.repository_owner}/nsp-code"
  }
...
module "container_image_ecr_server" {
  source  = "terraform-aws-modules/ecr/aws"
  version = "~> 1.4"

  repository_name = "nsp-server"
...
data  "aws_codestarconnections_connection" "ghckhc" {
  name          = "github-connection-kch"
...
module "codepipeline_ci_cd" {
  ...
  stage = [{
    ...
    action = [{
      ...
      configuration = {
        ConnectionArn    = data.aws_codestarconnections_connection.ghckhc.arn
        #OAuthToken           = data.aws_secretsmanager_secret_version.github_token.secret_string
       /* Owner                = var.repository_owner*/
        FullRepositoryId     = "KarolChlasta/${var.repository_name}"
